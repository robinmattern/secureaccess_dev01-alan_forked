...
class GlobalAuth {
    ...
    static async loadRoleByMemberId(memberId) {
        try {
//          const response = await fetch(     `http://localhost:3004/api/members?id=${memberId}`);          //#.(51013.04.11 IODD)
    ...
    static async loadRoleName() {
        try {
//          const response = await fetch(   'http://localhost:3004/api/webpage_roles_view');                //#.(51013.04.12 IODD ??)
            const response = await fetch(`${window.FVARS.SERVER_API_URL}/webpage_roles_view`)               // .(51013.04.12 IODD)
