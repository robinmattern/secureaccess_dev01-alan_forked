...
class GlobalAuth {
    ...
    static _rolesCache = null;
    static _rolesCacheTime = 0;
    ...
    static async loadUserByEmail(email) {
        try {
            const response = await fetch(`${window.FVARS.SERVER_API_URL}/user_by_email?email=${encodeURIComponent(email)}`);  // .(51013.04.10)
    ...
    static async loadRoleByMemberId(memberId) {
        try {
            const response = await fetch(`${window.FVARS.SERVER_API_URL}/members?id=${memberId}`)           // .(51013.04.11 IODD)
            const data = await response.json();
    ...
    static async loadRoleName() {
        try {
            ...
            const now = Date.now();
            if (!this._rolesCache || (now - this._rolesCacheTime) > this.CACHE_DURATION) {
                const response = await fetch(`${window.FVARS.SERVER_API_URL}/webpage_roles_view`);
                const roles = await response.json();
                this._rolesCache = Array.isArray(roles) ? roles : (roles.roles || []);
                this._rolesCacheTime = now;
            }
            
            const role = this._rolesCache.find(r => r.Id == this._gRoleId);
